*** Installation
To install backend, simply do the following. It first install dependencies and downloads the OuteAI small language model[1].
#+begin_src bash
pip install -r requirements.txt
uvicorn server:app --port 8081 --reload
#+end_src

And serve the index.html through any web server. For example, using ~http-server~.
#+begin_src bash
http-server -p 8001
#+end_src
...
So I first built a simple chat application with it. The github link is: ~TODO~.
[[file:chat application.png]]
